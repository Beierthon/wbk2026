...
function buildSummaryMessage(detections: VisionDetection[], rawDetections: DetectedObject[]) {
  if (detections.length === 0) {
    return "Keine Objekte erkannt. Kamera auf Gegenstaende richten oder Demo-Scan nutzen."
  }

  const chairCount = rawDetections.filter((detection) => detection.class === "chair").length
